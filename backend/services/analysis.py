...
try:
    ...
    if len(sys.argv) < 2:
        raise ValueError("Image path required as argument")
    ...
    # Calculate confidence score based on consistency of detected lines
    confidence = 0.0
    if len(row_lines) > 1 and len(col_lines) > 1:
        row_spacings = np.diff(row_lines)
        col_spacings = np.diff(col_lines)
        
        if len(row_spacings) > 0 and len(col_spacings) > 0:
            row_consistency = 1.0 / (1.0 + np.std(row_spacings) / np.mean(row_spacings))
            col_consistency = 1.0 / (1.0 + np.std(col_spacings) / np.mean(col_spacings))
            confidence = min(1.0, (row_consistency + col_consistency) / 2.0)

    # Calculate average grid size
    avg_grid_size = (row_spacing + col_spacing) / 2

    # Output results as JSON
    import json
    result = {
        "gridHeight": int(num_rows),
        "gridWidth": int(num_cols),
        "gridSize": round(avg_grid_size),
        "rowSpacing": round(row_spacing, 2),
        "colSpacing": round(col_spacing, 2),
    ...
    error_result = {
        "success": False,
        "error": str(e),
        "gridHeight": 10,  # Default fallback values
        "gridWidth": 10,
        "gridSize": 40,
        "confidence": 0.0
    }
